...
//This holds the key/value pairs for URLs we might want to access and what should be done once accessed.
const urls = {
...
const onRequest = (req, res) => {
  ...
  if (req.method === 'POST') {  //If we are attempting a post, send it to our function to handle post requests
    handlePOST(req, res, reqURL);
  } else if (req.method === 'HEAD') handleHEAD(req, res, reqURL); //Send it to our function to handle HEAD requests
  else if (urls[reqURL.pathname]) urls[reqURL.pathname](req, res, reqURL);  //Else try to access the proper handler from our URL struct
